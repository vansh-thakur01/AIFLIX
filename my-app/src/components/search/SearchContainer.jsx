        ...
        <div className="mt-[8%]">
            <div className="flex ml-18 mb-10">
                ...
                <div className="flex flex-wrap gap-y-2 w-[1500px]">
                    {moviesData.slice(0,len).map(movie => <p className=" text-xl text-white/80 border-r-2 pl-3 pr-4">{movie.title}</p>)}
                </div>
            </div>
            <div className="flex justify-center">
                <div className="flex flex-wrap gap-y-18 w-[97%] h-[50%] pl-7  ">
                {moviesData.map((movie) => (
                    <div className="w-49.5"><MovieCard width="15px" posterPath={movie.poster_path} /></div>
                    ))}
                </div>
            </div>
        </div>
